...
const UserList = ({ token }) => {
  ...
  const fetchUsers = async (tk) => {
    try {
      ...
      const response = await axios.get('https://group3-project-1.onrender.com/users', {
        headers: {
  ...
  const handleAddUser = async (e) => {
    ...
    try {
      const tk = token || localStorage.getItem('token');
      await axios.post('https://group3-project-1.onrender.com/users', newUser, {
  ...
  const saveEdit = async (e) => {
    ...
    try {
      ...
      await axios.put(`https://group3-project-1.onrender.com/users/${editingUser._id}`, payload, {
        headers: { Authorization: `Bearer ${tk}` },
      });
      await fetchUsers(tk);
      setEditingUser(null);
  ...
  const handleDelete = async (userId) => {
    if (window.confirm('Bạn có chắc chắn muốn xóa user này?')) {
      try {
        const tk = token || localStorage.getItem('token');
        await axios.delete(`https://group3-project-1.onrender.com/users/${userId}`, {
          headers: { Authorization: `Bearer ${tk}` }
        });
